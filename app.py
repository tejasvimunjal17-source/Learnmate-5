...
watsonx.ai for roadmap generation + OpenRouter for the AI Mentor chatbot).
"""
...
# Chat replies are now generated by OpenRouter (backend/openrouter_client.py)
# instead of the previous IBM watsonx Orchestrate widget - the widget itself
# (layout, floating button, styling) is unchanged.
...
with st.sidebar:
    ...
    # NOTE: kept in sync with NAV_OPTIONS_NEW below (the new-features menu) so
    # navigating to a new page doesn't get silently reset here every rerun.
    ALL_NAV_OPTIONS = NAV_OPTIONS + ["📚 Free Courses", "📄 Resume Builder"]
    if st.session_state["page"] not in ALL_NAV_OPTIONS:
    ...
    # -- New features (added below Dashboard) --------------------------
    # A second, independent option_menu widget so the original NAV_OPTIONS
    # menu above is left completely untouched. It uses its own widget key
    # and compares against a dedicated "last seen" session_state value
    # (NOT against st.session_state["page"]) - comparing directly against
    # "page" would cause the two menus to fight each other on every rerun,
    # since each keeps its own persisted selection across reruns even when
    # the other menu is the one that actually changed the page.
    NAV_OPTIONS_NEW = ["📚 Free Courses", "📄 Resume Builder"]
    NAV_ICONS_NEW = ["book", "file-earmark-person"]

    new_default_index = (
        NAV_OPTIONS_NEW.index(st.session_state["page"])
        if st.session_state["page"] in NAV_OPTIONS_NEW else 0
    )
    selected_new = option_menu(
        menu_title=None,
        options=NAV_OPTIONS_NEW,
        icons=NAV_ICONS_NEW,
    ...
    if "_secondary_nav_prev" not in st.session_state:
        st.session_state["_secondary_nav_prev"] = selected_new
    elif selected_new != st.session_state["_secondary_nav_prev"]:
        st.session_state["_secondary_nav_prev"] = selected_new
        st.session_state["page"] = selected_new
        st.rerun()
