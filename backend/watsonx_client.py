...
class WatsonxClient:
    ...
    def _extract_error_detail(resp: "requests.Response") -> str:
        """Pull IBM's actual error message/code out of the JSON error body.

        watsonx.ai error responses normally look like:
            {"errors": [{"code": "...", "message": "..."}], "trace": "..."}
        Falls back to raw response text if the body isn't in that shape.
        """
        try:
            payload = resp.json()
            ...
            if errors:
                first = errors[0]
                code = first.get("code", "unknown_code")
    ...
    def generate_text(
        ...
        if resp.status_code >= 400:
            ...
            logger.error(
                "watsonx.ai error [%s] url=%s model_id=%s project_id=%s: %s",
                resp.status_code, url, self.config.model_id,
                self.config.project_id, resp.text[:500],
            )
            if resp.status_code == 404:
                raise WatsonxError(
                    "watsonx.ai returned 404 Not Found. This almost always means "
                    "your WATSONX_URL region doesn't match the region your project "
                    "actually lives in (Dallas/us-south, Frankfurt/eu-de, "
                    "London/eu-gb, Tokyo/jp-tok, Sydney/au-syd, Toronto/ca-tor), "
                    "or the Project ID / Model ID is wrong for that region. "
                    f"IBM's response: {detail}"
                )
            raise WatsonxError(
