...
def generate_roadmap(profile: StudentProfile) -> dict[str, Any]:
    ...
    try:
        ...
        # Broadened deliberately beyond (WatsonxError, ValueError): network
        # errors, auth errors, malformed responses, or even an import-path
        # edge case that produces a WatsonxError instance that isn't an
        # `isinstance` match here should ALL degrade to the offline roadmap
        # rather than crash the page. Roadmap generation must never be a
        # hard failure for the user - see requirement to always fall back.
        logger.warning(
